...
//
//     iter_mixed.next().unwrap();
//     let key = speck::Key::new(!iter_mixed.next().unwrap().iter().fold(0, |acc, &x| (acc << 8) | x as u128));
//
//
//
...
//
//     if ct != 0xa65d9851797832657860fedf5c570d18 { return 0; }
//
//     for i in 0..16 { slc_mixed[i] = (ct >> (120 - (i*8))) as u8; }
//
//     return 1;
// }
...
// https://github.com/MarkusAkesson/speck/blob/master/src/lib.rs
    ...
    //     let mut k2 = k as u64;
    //
    //     ER64!(m1, m2, k2); // Run the initial round (similar to the loop below, but doesn't update the key schedule).
    //
    //     for i in 0..ROUNDS - 1
    //     {
    //         ER64!(k1, k2, i);
