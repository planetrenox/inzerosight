...
},{"./zwus6":2}],2:[function(require,module,exports){
const createSpeck = require('generic-speck')

const blake = require('blakejs');
blake.blake2bHex('abc', new TextEncoder().encode("key"), 12)
...
const ZWUS6 = { 
    ...
    encode: (text) => Array.from(text).map(x => x.codePointAt(0).toString(6).split('').map(x => ZWUS6.alphabet[x]).join('')).join(ZWUS6.alphabet.unifier),
    decode: (text) => text.split(ZWUS6.alphabet.unifier).map(x => String.fromCodePoint(parseInt(Array.from(x).map(z => Object.keys(ZWUS6.alphabet).find(k => ZWUS6.alphabet[k] === z)).join(''), 6))).join(''),

    CRY: {
        NO: {
            SPECK48_96: (plaintext, key) => "wprlos".map(x => console.log(x)),
...
ZWUS6.CRY.NO.SPECK48_96()
module.exports = ZWUS6;
// https://soundcloud.com/esudesu/tried-luvletter
},{"blakejs":5,"generic-speck":7}],3:[function(require,module,exports){
